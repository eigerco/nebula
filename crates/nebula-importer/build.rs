...
fn main() {
    ...
    println!("cargo:rerun-if-changed=Nebula.toml");
    let config: Config = Figment::new()
        .merge(Toml::file("Nebula.toml"))
        .extract()
        .expect("Could not read config in `Nebula.toml`.");
...
fn sync_contracts(config: &Config, cache: &PathBuf) -> anyhow::Result<()> {
